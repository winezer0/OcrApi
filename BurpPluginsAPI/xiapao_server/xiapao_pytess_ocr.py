...
def img_url_ocr():
    ...
    try:
        ...
        img_path = store_img_content(img_bytes)
        ocr_text = pytess_ocr(img_path)
        # 保存最新count个的验证码及识别结果
...
def pytess_ocr(img_path):
    ...
    try:
        # result = pytesseract.image_to_string(img_path, lang='eng', config="--psm 7").strip()
        # 设置Tesseract配置，仅识别数字且期望文本长度为4
        custom_config = r'--oem 3 --psm 8 -c tessedit_char_whitelist=0123456789'
        result = pytesseract.image_to_string(img_path, lang='eng', config=custom_config).strip()
        print(f"repid_ocr 识别结果:{result}")
        return result
    except Exception as error:
