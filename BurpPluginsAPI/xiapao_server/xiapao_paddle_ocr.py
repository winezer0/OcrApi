...
from xp_utils import save_latest_entries, send_http_package, guess_captcha_format, decode_b64, \
    get_log_content, store_img_content, extract_tuple_in_list
...
def paddle_ocr(img_path):
    ...
    result = engine.ocr(img_path, cls=False)
    if result:
        text = extract_tuple_in_list(result)
        print(f"paddle_ocr 识别结果:{result} -> {text[0]}")
        return text[0]
    else:
        return "0000"
